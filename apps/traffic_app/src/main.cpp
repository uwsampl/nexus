 ...
 public:
  ...
  void Setup() final {
    ssd_model_ = GetModelHandler("tensorflow", "ssd_mobilenet", 1,
                                 ssd_latency_ms_, 0, {}, 0);
    car_model_ = GetModelHandler("caffe2", "googlenet_cars", 1, 0);
    face_model_ = GetModelHandler("caffe2", "vgg_face_0", 1, 0);
    //build relationship graph;
    LoadDependencyRequest request;
    request.set_n(3);
    request.set_m(2);
    request.set_latency = latency_slo;
    auto ssd = ssd_model_.get_model_session();
    auto car = car_model_.get_model_session();
    auto face = face_model_.get_model_session();
    request.add_models(ssd);
    request.add_models(car);
    request.add_models(face);
    Edge edge;
    edge.set_v1(ssd);
    edge.set_v2(car);
    request.add_edges(edge);
    Edge edge;
    edge.set_v1(ssd);
    edge.set_v2(face);
    request.add_edges(edge);
